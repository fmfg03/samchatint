...
def _detect_group_by(text: str) -> str:
    if any(token in text for token in ("cuenta contable", "account", "cuenta")):
        return "account"
...
def detect_finance_comparison_intent(text: str) -> Optional[FinanceComparisonIntent]:
    ...
    has_comparison = any(
        token in normalized
        for token in ("compara", "comparar", "contra", " vs ", "variacion", "variacion")
    )
