...
AnalystProviderFn = Callable[[AnalystIntent, List["AnalystEvidence"]], Awaitable[str]]
...
def _document_intake_evidence(content: str) -> Optional[AnalystEvidence]:
    ...
    missing = [str(item) for item in (intake.get("missing_fields") or []) if item]
    if missing:
...
def extract_analyst_evidence_from_messages(messages: Iterable[Any]) -> List[AnalystEvidence]:
    ...
    for item in messages:
        ...
        if intake_evidence is not None:
            ...
            continue
        if "|" in content and any(token in content.lower() for token in ("comparacion", "reporte", "cfdi", "pagos")):
            evidence.append(
                AnalystEvidence(
...
def _needs_context(intent: AnalystIntent) -> AnalystWorkbenchResult:
    ...
    return AnalystWorkbenchResult(
        ...
        evidence=[],
        caveats=["Sin contexto no puedo sostener una conclusión sin inventar datos."],
        next_questions=[
...
def _routed_to_operational(intent: AnalystIntent) -> AnalystWorkbenchResult:
    return AnalystWorkbenchResult(
        ...
        answer="Esta solicitud corresponde a una ruta operacional determinística, no al Analyst Workbench.",
        evidence=[],
        caveats=[],
        next_questions=[],
        suggested_routes=[intent.operational_route_hint or "request_intelligence"],
        actions_executed=[],
        provider_called=False,
    )


def _answer_for_intent(intent: AnalystIntent, evidence: List[AnalystEvidence]) -> tuple[str, List[str], List[str]]:
    ...
    if intent.analyst_intent == "risk_review":
        return (
            ...
            "- Riesgo de evidencia incompleta si faltan anexos, importes, fechas o responsables.\n"
            "- Riesgo operativo si las obligaciones no tienen dueño, fecha límite o criterio de aceptación.\n"
            "- Riesgo financiero si los montos no están reconciliados contra CFDI, pago o presupuesto.",
            ["El análisis se limita al contexto disponible; no revisé datos vivos adicionales."],
            [
                "¿Existe anexo, SOW o contrato completo para validar obligaciones?",
    ...
    if intent.analyst_intent == "compare":
        return (
            ...
            "- Puntos a contrastar: alcance, fechas, entregables, costos, penalizaciones y responsables.\n"
            "- Para una comparación completa necesito ambos documentos o sus extractos relevantes.",
            ["Si solo hay un documento en contexto, la comparación queda incompleta."],
            ["¿Cuál es el documento base y cuál es la versión/propuesta a comparar?"],
        )
    if intent.analyst_intent == "questions":
        return (
            ...
            ["Estas preguntas salen del contexto disponible, no de datos externos."],
    ...
    if intent.analyst_intent == "next_steps":
        return (
            ...
            "- Separar pendientes por dueño, fecha límite y evidencia requerida.\n"
            "- Identificar bloqueos que requieren decisión de dirección o cliente.\n"
            "- Definir el siguiente entregable verificable.",
            ["No ejecuté acciones; son pasos sugeridos para revisión humana."],
            ["¿Cuál es la fecha objetivo de cierre?", "¿Quién aprueba el siguiente entregable?"],
        )
    if intent.analyst_intent == "summarize":
        return (
            "Resumen con el contexto disponible:\n"
            f"- {primary}\n"
            "- No hay evidencia suficiente para afirmar hechos fuera de ese contexto.\n"
            "- Conviene validar faltantes antes de usarlo como conclusión ejecutiva.",
    ...
    return (
        ...
        f"- {primary}\n"
        "- Lo anterior describe la evidencia disponible, no una validación contra datos vivos.\n"
        "- Si necesitas una conclusión formal, faltaría confirmar fuente, fecha y alcance.",
    ...
    if provider_allowed and provider_fn is not None:
        try:
            ...
            return AnalystWorkbenchResult(
                ...
                answer="No pude usar provider para redactar el análisis. No ejecuté acciones ni cambios.",
                evidence=[item.to_dict() for item in evidence],
                caveats=["Provider no disponible; no se inventó respuesta."],
                next_questions=["¿Quieres que responda solo con síntesis determinística del contexto?"],
                suggested_routes=[],
                actions_executed=[],
