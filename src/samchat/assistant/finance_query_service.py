...
def build_comparison_rows(
    *,
    ...
    for row in source_rows:
        ...
        grouped[label][year] = round(grouped[label][year] + _amount_from_row(row), 2)
...
def render_finance_comparison_result(result: FinanceComparisonResult) -> str:
    ...
    lines = [
        f"Comparación de gasto por {intent.get('group_by')}, {current_year} vs {base_year}",
        "",
    ...
    for row in result.rows:
        ...
        lines.append(
            "| {label} | ${base:,.2f} | ${current:,.2f} | ${diff:,.2f} | {variation} |".format(
    ...
    *,
    ...
    try:
        if rows_provider is not None:
            ...
            source_rows = await _read_expense_rows(session=session, intent=intent)
    ...
    rows = [row.to_dict() for row in build_comparison_rows(intent=intent, source_rows=source_rows)]
    if not rows:
        years = intent.years
        return FinanceComparisonResult(
            status="empty",
            message=(
                f"No encontré datos suficientes para comparar gasto {years[0]} vs "
