...
class PhotoRegion(BaseModel):
    ...
    x: int = Field(
        ...,
        ge=0,
        description="X coordinate of top-left corner (pixels from left edge)"
    )

    y: int = Field(
        ...,
        ge=0,
        description="Y coordinate of top-left corner (pixels from top edge)"
    )

    width: int = Field(
        ...,
        ge=10,
        description="Width of the photo region in pixels"
    )

    height: int = Field(
        ...,
        ge=10,
        description="Height of the photo region in pixels"
    )
...
class PlayerExtraction(BaseModel):
    ...
    Player slots may be partial. Other fields are optional but will be
    ...
    paternal_surname: Optional[str] = Field(
        None,
        max_length=50,
        description="Apellido paterno (father's surname)"
    )

    maternal_surname: Optional[str] = Field(
        None,
        max_length=50,
        description="Apellido materno (mother's surname)"
    )

    first_name: Optional[str] = Field(
        None,
        max_length=50,
        description="Nombre(s) de pila (given name)"
    )
    ...
    needs_review: bool = Field(
        default=False,
        description="Flag if this player's data needs human review"
    )
            ...
            if match:
                ...
                try:
                    if len(groups[0]) == 4:  # YYYY-MM-DD format
                        return fmt.format(int(groups[0]), int(groups[1]), int(groups[2]))
...
class TeamExtraction(BaseModel):
    ...
    category: Optional[str] = Field(
        None,
        description="Tournament category (U10, U12, U14, U16, U18, Open, Juvenil)"
    )

    gender: Optional[str] = Field(
        None,
        description="Team gender: varonil, femenil, or mixto"
    )

    league: Optional[str] = Field(
        None,
        max_length=100,
        description="League name (nombre de la liga)"
    )

    municipality: Optional[str] = Field(
        None,
        max_length=100,
        description="Municipality/city (municipio)"
    )
    ...
    confidence: float = Field(
        ...
        ge=0.0,
        le=1.0,
        description="Confidence score for team extraction"
    )
...
class ManagerExtraction(BaseModel):
    ...
    name: str = Field(
        ...
        min_length=3,
        max_length=100,
        description="Manager/coach full name"
    )

    role: Optional[str] = Field(
        None,
        description="Role: manager, entrenador, delegado, representante"
    )
...
class ResponsableExtraction(BaseModel):
    ...
    role: Optional[str] = Field(
        None,
        description="Role: delegado, representante, entrenador, manager"
    )
    ...
    birth_date: Optional[str] = Field(
        None,
        description="Date of birth if visible (DD/MM/YYYY)"
    )

    curp: Optional[str] = Field(
        None,
        description="CURP if visible on the registration form"
    )

    needs_review: bool = Field(
        default=False,
        description="Flag if this responsable's data needs human review"
    )
...
class RegistrationFormExtraction(BaseModel):
    ...
    is_front: bool = Field(
        default=True,
        description="Whether this is the front side of the form"
    )
    ...
    form_type: Optional[str] = Field(
        None,
        description="Type of form detected: roster, individual, credential"
    )
    ...
    def calculate_needs_review(self) -> 'RegistrationFormExtraction':
        ...
        low_confidence_players = sum(
            ...
            if p.confidence < 0.7 or p.needs_review
        )
...
class SinglePlayerExtraction(BaseModel):
    ...
    team_name: Optional[str] = Field(
        None,
        description="Team name if visible"
    )

    category: Optional[str] = Field(
        None,
        description="Tournament category if visible"
    )

    photo_detected: bool = Field(
        default=False,
        description="Whether a player photo was detected in the form"
    )
...
class OCRExtractionResult(BaseModel):
    ...
    extraction_type: Literal["registration_form", "single_player", "roster", "unknown"] = Field(
        default="unknown",
    ...
    data: Optional[RegistrationFormExtraction | SinglePlayerExtraction] = Field(
        None,
        description="Extracted data based on extraction_type"
    )

    raw_text: Optional[str] = Field(
        None,
        description="Raw transcribed text from the image"
    )
