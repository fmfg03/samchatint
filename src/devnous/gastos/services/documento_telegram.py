...
from ..utils.mexico_city_dates import to_mexico_city_datetime
...
def _fmt_dt(value: Optional[datetime]) -> str:
    if value is None:
        return "—"
    try:
        cdmx_value = to_mexico_city_datetime(value)
        if cdmx_value is None:
            return "—"
        return cdmx_value.strftime("%Y-%m-%d %H:%M CDMX")
    except Exception:
        return str(value)
...
def concepto_resumen(documento: Documento) -> str:
    ...
    if prov is not None and getattr(prov, "nombre", None):
        parts.append(f"Proveedor: {str(prov.nombre).strip()}")
...
def format_documento_resumen_es(
    ...
    *,
    ...
    ben = escape_markdown_light(beneficiario_label(documento))
