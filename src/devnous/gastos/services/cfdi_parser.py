...
def parse_cfdi_xml(xml_content: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if conceptos_elem is not None:
            for concepto in conceptos_elem.findall(
                ".//{http://www.sat.gob.mx/cfd/4}Concepto"
            ) or conceptos_elem.findall(".//Concepto"):
                concepto_data = {
                ...
                if impuestos_elem is not None:
                    traslados = []
                    for traslado in impuestos_elem.findall(
                        ".//{http://www.sat.gob.mx/cfd/4}Traslado"
                    ) or impuestos_elem.findall(".//Traslado"):
                        traslados.append(
                            {
                                ...
                                "tasa_o_cuota": float(
                                    traslado.get("TasaOCuota", 0) or 0
                                ),
        ...
        # Extract Impuestos at Comprobante level. A descendant search can select
        # the first Concepto/Impuestos node and undercount invoices with more
        # than one taxed concept.
        ...
        traslados_list = []
        if impuestos_elem is not None:
            traslados_elem = impuestos_elem.find(
            ...
            if traslados_elem is not None:
                for traslado in traslados_elem.findall(
                    ".//{http://www.sat.gob.mx/cfd/4}Traslado"
                ) or traslados_elem.findall(".//Traslado"):
        ...
        retenciones_list = []
        if impuestos_elem is not None:
            retenciones_elem = impuestos_elem.find(
                ".//{http://www.sat.gob.mx/cfd/4}Retenciones"
            )
            if retenciones_elem is None:
                retenciones_elem = impuestos_elem.find(".//Retenciones")

            if retenciones_elem is not None:
                for retencion in retenciones_elem.findall(
                    ".//{http://www.sat.gob.mx/cfd/4}Retencion"
                ) or retenciones_elem.findall(".//Retencion"):
        ...
        if conceptos_list and len(conceptos_list) > 0:
            descripcion_concepto_principal = str(
                conceptos_list[0].get("descripcion", "") or ""
            )
        ...
        result = {
            ...
            "emisor_regimen_fiscal": (
                emisor.get("RegimenFiscal", "") if emisor is not None else ""
            ),
            # Receptor
            "receptor_rfc": receptor.get("Rfc", "") if receptor is not None else "",
            "receptor_nombre": (
                receptor.get("Nombre", "") if receptor is not None else ""
            ),
            "receptor_uso_cfdi": (
                receptor.get("UsoCFDI", "") if receptor is not None else ""
            ),
            ...
            "fecha_timbrado": (
                parse_datetime(timbre.get("FechaTimbrado", ""))
                if timbre is not None
                else None
            ),
            "rfc_prov_certif": (
                timbre.get("RfcProvCertif", "") if timbre is not None else ""
            ),
            "sello_cfd": timbre.get("SelloCFD", "") if timbre is not None else "",
            "no_certificado_sat": (
                timbre.get("NoCertificadoSAT", "") if timbre is not None else ""
            ),
            "sello_sat": timbre.get("SelloSAT", "") if timbre is not None else "",
            # Impuestos
            "total_impuestos_trasladados": (
                float(impuestos_elem.get("TotalImpuestosTrasladados", 0) or 0)
                if impuestos_elem is not None
                else 0.0
            ),
