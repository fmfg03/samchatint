    ...
    if categoria in {"cfdi_xml", "cfdi_pdf"}:
        ...
        if remaining_cfdi.first() is None:
            ...
            if documento is not None:
                ...
                _agent_debug_log(
                    hypothesis_id="H2",
                    location="documento_service.py:remove_solicitud_documento_adjunto",
                    message="cleared documento cfdi link after last cfdi adjunto removed",
                    data={
                        "documento_id": str(documento_id),
                        "adjunto_id": str(adjunto_id),
                        "removed_categoria": categoria,
                        "previous_cfdi_report_id": str(documento.cfdi_report_id or ""),
                    },
                )
                # #endregion
