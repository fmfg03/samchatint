...
_DEBUG_LOG_PATH = "/root/samchat/.cursor/debug-a501a9.log"
...
async def _ingest_cfdi_parsed(
    ...
    *,
    ...
    if report is None:
        ...
        _agent_debug_log(
            hypothesis_id="H3-H4",
            location="cfdi_ingestion_service.py:_ingest_cfdi_parsed:existing",
            message="existing cfdi ingest decision",
            data={
                "cfdi_uuid": canonical_uuid,
                "conflicts": plan.conflicts,
                "apply_fiscal_data": plan.apply_fiscal_data,
                "overwrite": plan.overwrite,
                "pdf_attach_only": plan.pdf_attach_only,
                "existing_has_xml_raw": _report_has_canonical_xml(report),
                "incoming_has_xml_raw": bool(xml_raw and str(xml_raw).strip()),
                "same_entity_linked": _entity_linked_to_report(entity, report),
                "existing_fecha": getattr(report, "fecha", None),
                "incoming_fecha": parsed.get("fecha"),
        ...
        _agent_debug_log(
            hypothesis_id="H3-H4",
            location="cfdi_ingestion_service.py:_ingest_cfdi_parsed:existing_result",
            message="existing cfdi ingest result",
            data={
                "cfdi_uuid": canonical_uuid,
                "status": status,
                "changed": changed,
