...
def utc_now() -> datetime:
    """Timezone-aware UTC timestamp for DB columns (timestamptz)."""
...
def format_mexico_city_datetime(
    value: Optional[Union[datetime, Any]],
    *,
    fmt: str = "%Y-%m-%d %H:%M CDMX",
) -> str:
    """Format a datetime in Mexico City local time for user-facing copy."""
    cdmx_value = to_mexico_city_datetime(value)
    if cdmx_value is None:
        return "—"
    return cdmx_value.strftime(fmt)
