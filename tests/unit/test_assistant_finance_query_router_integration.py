...
@pytest.mark.asyncio
async def test_finance_comparison_short_circuits_provider_and_offers_export_on_rows():
    ...
    assert "| Hospedaje | $1,000.00 | $1,500.00 | $500.00 | 50.00% |" in response.assistant_message
...
@pytest.mark.asyncio
async def test_empty_finance_comparison_does_not_offer_export_or_provider_fallback():
    response = await _run_finance_message(
...
@pytest.mark.asyncio
async def test_unavailable_finance_source_does_not_offer_export_or_provider_fallback():
    response = await _run_finance_message(
        "variación de gastos por concepto entre 2025 y 2026",
        _unavailable_finance_rows,
    )

    assert "fuente de datos financiera disponible" in response.assistant_message
    assert "¿Quieres que te lo exporte" not in response.assistant_message
    trace = response.tool_trace[0]["finance_query_live_wiring"]
    assert trace["provider_called"] is False
    assert trace["writes_attempted"] is False
