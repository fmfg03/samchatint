...
@pytest.mark.asyncio
async def test_successful_read_only_comparison_computes_rows_and_exportable_result():
    intent = detect_finance_comparison_intent(
        "Compara gasto 2026 vs 2025 por concepto"
    )

    result = await run_read_only_comparison(intent=intent, rows_provider=_sample_rows)
    ...
    assert "| Hospedaje | $1,000.00 | $1,500.00 | $500.00 | 50.00% |" in rendered
...
async def test_empty_result_says_no_data_and_is_not_exportable():
    ...
    result = await run_read_only_comparison(intent=intent, rows_provider=_empty_rows)
    rendered = render_finance_comparison_result(result)
