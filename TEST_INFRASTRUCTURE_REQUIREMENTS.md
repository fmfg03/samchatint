...
Host: localhost
Port: 5432
Connection String: postgresql://testuser:testpass@localhost:5432/tournament_test
```
...
Port: 6379
Password: testpass
Connection String: redis://:testpass@localhost:6379/0
```
...
```bash
export POSTGRESQL_URL="postgresql://testuser:testpass@localhost:5432/tournament_test"
```

Or update `/root/samchat/src/devnous/config.py`:
```python
postgresql_url: str = Field(
    default="postgresql://testuser:testpass@localhost:5432/tournament_test",
    env="POSTGRESQL_URL"
)
```
...
Ensure your environment has:
```bash
export REDIS_URL="redis://:testpass@localhost:6379/0"
```

Or update `/root/samchat/src/devnous/config.py`:
```python
redis_url: str = Field(
    default="redis://:testpass@localhost:6379/0",
    env="REDIS_URL"
)
```
...
**Solution**:
1. Check config uses correct credentials (testuser/testpass)
2. Verify PostgreSQL is running: `sudo systemctl status postgresql`
3. Test connection: `psql postgresql://testuser:testpass@localhost:5432/tournament_test`
...
2. Verify password in redis.conf: `requirepass testpass`
3. Test connection: `redis-cli -a testpass PING`
  ...
  test:
    ...
    steps:
      ...
      - name: Run integration tests
        env:
          POSTGRESQL_URL: postgresql://testuser:testpass@localhost:5432/tournament_test
