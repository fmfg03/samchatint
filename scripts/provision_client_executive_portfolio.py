...
from devnous.gastos.services.project_authorization_service import (  # noqa: E402
    ensure_project_authorization_schema,
)
...
async def _run(args: argparse.Namespace) -> int:
    ...
    receipt: dict[str, Any] = {
        ...
        "position_keys": sorted(set(args.position_key)),
    ...
    try:
        async with maker() as session:
            if args.apply:
                ...
                for position_key in receipt["position_keys"]:
                    await session.execute(text("""INSERT INTO client_executive_portfolio_positions
                    (portfolio_id, position_key) VALUES (:portfolio_id, :position_key)
                    ON CONFLICT (portfolio_id, position_key) DO UPDATE SET active = TRUE"""),
                    {"portfolio_id": args.portfolio_id, "position_key": position_key})
...
def main() -> int:
    ...
    parser.add_argument("--position-key", action="append", required=True)
