...
No production server, database, secrets, or user session was accessed during the
original sweep. No local test suite was executed because the cloned repository
did not include its project virtual environment. GitHub reported successful
`Assistant Scoped Gate` and `Test Suite` runs for the PR #306 head and for the
then-current #304 head. This canon update records that #304 later merged and
deployed; business UAT remains separate evidence.
...
- #304 merged and deployed as `dcbeea8423030e0123101a35378d765ea3c96f32`:
  requires the linked approved `INFORME` to carry `budget_concept_id` before
  promotion.

Risk: deployment of #304 does not replace historical reconciliation,
reference-by-reference verification, or Finance UAT before the incident is
described as business-closed.
...
- Has the deployed #304 guard been verified against every affected historical reference?
...
1. Preserve evidence for the deployed #304 guard and verify its release against the canonical drop-in.
2. Run the reimbursement reconciliation and produce a reference-by-reference before/after receipt.
3. Execute the Finance UAT document with real users and preserve evidence.
4. Freeze a client-facing scope matrix: accepted, accepted with observations, pending defect, requested change, and out of scope.
5. Close remaining SOW lanes explicitly: FMF, global dedup, scheduling/logistics, live ERP integration, sponsor approval/publication, proof-of-performance, onboarding/handoff, and support.
6. Update roadmap docs whose statuses now understate AR, cashflow, and Sam Inbox.
