...
| `samchat.assistant.tournament_goal_*` | Partial, reconciled with SOUL | Keep as persistent case/diff layer | Covers cloning/planning/diff logic. It now emits a SOUL Wizard handoff payload so tournament creation remains one story: case/diff first, wizard detail next. |
