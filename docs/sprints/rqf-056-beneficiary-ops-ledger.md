...
| RQF-057E - No deducibles accounting rule | CLOSED_LOCAL_PENDING_COMMIT | pending commit | SOLICITUD payment-generated expenses now receive project-specific No Deducibles account when no CFDI is linked; Informe line path already had project-specific suggestion/preview coverage. Evidence: `docs/sprints/rqf-057e-no-deducibles-rule.md`. |
...
1. Continue customer Excel delta in order: RQF-057F SAT download/reconciliation. Keep RQF-057G Authorization strategy matrix paused at advisory/board scope unless the customer asks to resume it.
