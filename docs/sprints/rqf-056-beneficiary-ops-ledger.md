...
| RQF-057D - Solicitudes search by proveedor | CLOSED_COMMITTED | 4c720f8ed | `/gastos-terceros` now shows a dedicated consultation filter bar above the solicitudes table with Referencia Operaciones, `Por Proveedor`, and Concepto. Test verifies `terceros-search-proveedor`, `data-proveedor`, normalization, and `matchProveedor`. Evidence: `docs/sprints/rqf-057d-provider-search.md`. |
| RQF-057E - No deducibles accounting rule | CLOSED_COMMITTED | 5541beacf | SOLICITUD payment-generated expenses now receive project-specific No Deducibles account when no CFDI is linked; Informe line path already had project-specific suggestion/preview coverage. Evidence: `docs/sprints/rqf-057e-no-deducibles-rule.md`. |
| RQF-057F - SAT massive download and reconciliation | CLOSED_LOCAL_PENDING_EXTERNAL_CREDENTIALS | this commit | Existing SAT connector separates recibidos/emitidos and has matching surface; this cut adds 09:00/23:00 and hourly runner scripts, admin/docstring schedule visibility, and contract tests. Live SAT witness remains pending FIEL/SAT credentials and cron installation. Evidence: `docs/sprints/rqf-057f-sat-massive-download.md`. |
| RQF-057G - Authorization strategy matrix | PARTIAL_COMMITTED_LOCAL | this commit | Foundation resolver created from customer matrix and Francisco role mapping; board registered in Control de accesos as configuracion.estrategias_autorizacion; profile board implemented; send-time advisory evidence is persisted and now visible in document detail; document detail also compares the suggested route against actual approvals as preview-only evidence; drafts now show a pre-send advisory preview using the same evidence builder; approvals now persist and surface non-blocking authorization-route soft warnings when actual approvals do not cover matrix roles; a read-only warnings dashboard is available for finance/admin review. Hard enforcement readiness now exists behind `SAMCHAT_AUTHORIZATION_STRATEGY_ENFORCEMENT`; sequential first/second approval gates remain pending. Evidence: docs/sprints/rqf-057g-authorization-strategy-matrix.md, docs/sprints/rqf-057g2-authorization-profile-board.md, docs/sprints/rqf-057g3-authorization-evidence.md, docs/sprints/rqf-057g4-authorization-evidence-detail.md, docs/sprints/rqf-057g5-authorization-route-preview.md, docs/sprints/rqf-057g6-authorization-pre-send-preview.md, docs/sprints/rqf-057g7-authorization-soft-warning.md, docs/sprints/rqf-057g8-authorization-warnings-dashboard.md, docs/sprints/rqf-057g9-authorization-enforcement-readiness.md. |
