...
| RQF-057D - Solicitudes search by proveedor | CLOSED_VERIFIED_LOCAL | existing implementation + focused test | `/gastos-terceros` already includes provider search beside Referencia Operaciones and Concepto; test verifies `terceros-search-proveedor`, `data-proveedor`, normalization, and `matchProveedor`. Evidence: `docs/sprints/rqf-057d-provider-search.md`. |
